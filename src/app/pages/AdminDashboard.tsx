        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 shadow-2xl space-y-5">
          <div>
            ...
            <input type="text" value={username} onChange={(e) => { setUsername(e.target.value); setError(""); }} onKeyDown={(e) => e.key === "Enter" && handleSubmit()}
              className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-white font-medium focus:outline-none focus:border-lime-400 focus:ring-1 focus:ring-lime-400 transition-all"
          ...
          <div>
            ...
            <div className="relative">
              <input type={showPass ? "text" : "password"} value={password} onChange={(e) => { setPassword(e.target.value); setError(""); }} onKeyDown={(e) => e.key === "Enter" && handleSubmit()}
                className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 pr-12 text-white font-medium focus:outline-none focus:border-lime-400 focus:ring-1 focus:ring-lime-400 transition-all"
          ...
          <button onClick={handleSubmit} disabled={loading}
            className="w-full bg-lime-400 hover:bg-lime-500 disabled:opacity-60 disabled:cursor-not-allowed text-zinc-950 font-black py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all active:scale-95">
...
export function AdminDashboard() {
  ...
  const stats = [
    ...
    { label: "Active Connections", value: "1,204", icon: <Zap size={20} className="text-lime-400" /> },
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "completed":   return <span className="bg-lime-400/10 text-lime-400 border border-lime-400/20 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1"><CheckCircle2 size={12} /> Completed</span>;
      default: return null;
    }
  };

  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-50 pt-24 pb-24 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-[600px] h-[600px] bg-lime-400/5 rounded-full blur-[150px] pointer-events-none -z-10" />
        ...
        <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 bg-zinc-900 border border-zinc-800 px-3 py-1 rounded-full text-zinc-400 text-xs font-bold uppercase tracking-wide mb-3">
              <Activity size={14} className="text-lime-400" /> Operations Centre
            </div>
            <h1 className="text-4xl font-black tracking-tighter">Admin Dashboard</h1>
            <div className="mt-2 inline-flex items-center gap-2 bg-lime-400/10 border border-lime-400/20 px-3 py-1 rounded-full text-lime-400 text-xs font-bold">
          ...
          <div className="flex gap-3">
            ...
            <button className="bg-lime-400 hover:bg-lime-500 text-zinc-950 px-4 py-2 rounded-xl font-bold text-sm transition-transform active:scale-95">New Installation</button>
            ...
            <div className="flex gap-2 w-full md:w-auto overflow-x-auto scrollbar-hide">
              {["installations", "engineers", "network"].map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  className={`px-5 py-2 rounded-full text-sm font-bold capitalize whitespace-nowrap transition-colors ${activeTab === tab ? "bg-lime-400 text-zinc-950" : "bg-zinc-950 text-zinc-400 border border-zinc-800 hover:text-white"}`}>
                  {tab}
                </button>
              ))}
            </div>
            <div className="flex gap-3 w-full md:w-auto">
              <div className="relative flex-1 md:w-64">
                ...
                <input type="text" placeholder="Search jobs..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full bg-zinc-950 border border-zinc-800 rounded-xl pl-9 pr-4 py-2 text-sm focus:outline-none focus:border-lime-400 focus:ring-1 focus:ring-lime-400 transition-all text-white placeholder:text-zinc-600" />
              </div>
          ...
          <AnimatePresence mode="wait">
                    ...
                    <tbody className="divide-y divide-zinc-800/50">
                      {filtered.length > 0 ? filtered.map((job) => (
                        <tr key={job.id} className="hover:bg-zinc-800/20 transition-colors group">
                          ...
                          <td className="p-4 text-right"><button className="text-zinc-500 hover:text-lime-400 transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100"><ChevronRight size={20} /></button></td>
                        </tr>
                      )) : (
            ...
            {activeTab === "network" && (
              <motion.div key="network" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-12 text-center text-zinc-500 flex flex-col items-center justify-center">
                <div className="w-16 h-16 bg-zinc-950 rounded-full flex items-center justify-center mb-4 text-lime-400/50"><Cable size={32} /></div>
